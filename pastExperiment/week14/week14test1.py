...
from tools.benchmark2013 import functions1

from cmaes import SepCMA

# line search on CMAES using median point and using best X as original point of next run Perturbation
def main():
    ...
    m = 30
    start = random.uniform(-100, 0)  # original search space
    stop = random.uniform(0, 100)
    ...
    inputX = bestX
    for e in range(30):
        inputX[e] = inputX[e] + random.uniform(-0.5, 0.5)

    for f in range(0, 30):
        print(f)
        optimizer = SepCMA(mean=inputX, sigma=20.0)  # CMAES part
        print(" evals    f(x)")
        print("======  ==========")
        ...
        while True:
            ...
            for _ in range(optimizer.population_size):
                ...
                evals += 1
                if round(value,7) == round(bvalue,7):
                    for n in range(30):
                        x[n] = x[n] + random.uniform(-0.5, 0.5)

                solutions.append((x, value))
                if evals % 3000 == 0:  #choose 3000 or 30000 to show return value
        ...
        for v in range(30):
            inputX[v] = inputX[v] + random.uniform(-0.5, 0.5)
